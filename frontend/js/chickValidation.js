...
chickRequestForm.addEventListener('submit', (e) => {
    ...
    apiFetch('/api/chick-requests', {
        method: 'POST',
        body: JSON.stringify({ farmerName, farmerNIN, numChicks, typeChicks, farmerType })
    })
      .then(() => {
        alert('Request submitted successfully!');
        chickRequestForm.reset();
      })
      .catch(err => alert(err.message || 'error try again'));
});
}
...
chickFeedsForm.addEventListener('submit', (e) => {
    ...
    apiFetch('/api/feeds-requests', {
        method: 'POST',
        body: JSON.stringify({
            farmerName: farmerFName,
            farmerNIN: farmerFNIN,
            quantityFeeds: feedsFQuantity,
            typeFeeds: typeFFeeds,
            farmerType: farmerFType
        })
    })
      .then(() => {
        alert('Request submitted successfully!');
        chickFeedsForm.reset();
      })
      .catch(err => alert(err.message || 'error try again'));
});
}
