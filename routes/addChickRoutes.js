...
router.post('/', async (req, res) => {
    try {
        ...
        if (existingRequest) {
            return res.status(409).send('cant make a request now')
        }
        await newRequest.save()
        return res.redirect('/chickRequest')
...
// router.post('/update-chick-request/:id', async (req, res) => { 
//     console.log("########");
//     try {
//         const { id } = req.params;
//         const {
...
//         res.redirect('/dashboard?success=Request updated successfully');
//     } catch (error) {
//         console.error('Update error:', error);
//         res.redirect('/dashboard?error=Failed to update request');
//     }
// });


// Feeds Request routes are handled in routes/feedsRoutes.js
